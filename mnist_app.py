...
from streamlit_mnist import Testing, multiple_classification
...
def MNIST():
    ...
    st.write('draw multiple digits then classification')
    ...
    if canvas_result2.image_data is not None:
        # st.image(canvas_result.image_data[:,:,1])
        image = np.array(canvas_result2.image_data[:, :, 1] / 255).astype('float32')

        multi_img, multi_predict = multiple_classification(image)
        st.image(multi_img)
        # st.write(multi_predict)
        result2 = f'<p style="font-family:sans-serif; color:red; font-size: 42px;">{multi_predict}</p>'
        st.markdown(result2, unsafe_allow_html=True)
