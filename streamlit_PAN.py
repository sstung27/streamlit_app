...
def reconImg(arrayClear, block_numM, block_numN, block_size, block_extend, h, w, m1, n1):
    ...
    img = np.zeros((h1, w1)).astype('float32')
    index = np.zeros((h1, w1)).astype('float32')
    count = 0
    for ii in range(0, h1 - block_size + 1, block_size):
        for jj in range(0, w1 - block_size + 1, block_size):
...
def doing_fusion_PAN(image1, image2):
    fuse_scheme = 0
    if fuse_scheme == 0:
    ...
    model.load_state_dict(torch.load('sigma1_epoch50.pth'))
    model.eval()
    ...
    cv2.imwrite(route_fuse, imgFuse)
    stop_time = time.time()
